...
#include <set>
#include <vector>  //89da36: for sib1_fragments storage
...
public:
  ...
  /// Add a SIB1 fragment to the collection (for multiple SIB1 support)
  bool add_sib1_fragment(const asn1::rrc_nr::sib1_s& sib1_);
  /// Get the number of SIB1 fragments collected
  size_t get_nof_sib1_fragments() const { return sib1_fragments.size(); }
  /// Check if all expected SIB1 fragments have been collected
  bool has_all_sib1_fragments(unsigned expected_count) const { return sib1_fragments.size() >= expected_count; }
  /// Get a specific SIB1 fragment by index
  const asn1::rrc_nr::sib1_s* get_sib1_fragment(size_t idx) const;
  //89da36:end
  ...
  //89da36:start - Storage for multiple SIB1 fragments
  std::vector<asn1::rrc_nr::sib1_s> sib1_fragments;  // Store all received SIB1 fragments
  //89da36:end
};
