...
void meas_cell_nr::set_sib1(const asn1::rrc_nr::sib1_s& sib1_)
{
  sib1           = sib1_;
  has_valid_sib1 = true;
  // Note: Don't call add_sib1_fragment here - it should be called explicitly
  // by the caller before set_sib1 to avoid duplicate detection issues
}

//89da36:start - Support for multiple SIB1 fragments
bool meas_cell_nr::add_sib1_fragment(const asn1::rrc_nr::sib1_s& sib1_)
{
  // Check if this fragment is already in the collection by comparing q_rx_lev_min
  // (since we modify this field to distinguish fragments)
  int8_t new_q_rx_lev_min = sib1_.cell_sel_info_present ? sib1_.cell_sel_info.q_rx_lev_min : 0;
  
  srslog::basic_logger& logger = srslog::fetch_basic_logger("RRC");
  logger.info("add_sib1_fragment: current_fragments=%zu, new_q_rx_lev_min=%d", 
              sib1_fragments.size(), new_q_rx_lev_min);
  
  for (const auto& existing : sib1_fragments) {
    int8_t existing_q_rx_lev_min = existing.cell_sel_info_present ? existing.cell_sel_info.q_rx_lev_min : 0;
    logger.debug("add_sib1_fragment: comparing with existing fragment, q_rx_lev_min=%d", existing_q_rx_lev_min);
    if (existing_q_rx_lev_min == new_q_rx_lev_min) {
      // This fragment is already in the collection
      logger.info("add_sib1_fragment: duplicate detected (q_rx_lev_min=%d), returning false", new_q_rx_lev_min);
      return false;
    }
  }
  
  // This is a new fragment, add it
  sib1_fragments.push_back(sib1_);
  logger.info("add_sib1_fragment: added new fragment, total_fragments=%zu", sib1_fragments.size());
  return true;
}

const asn1::rrc_nr::sib1_s* meas_cell_nr::get_sib1_fragment(size_t idx) const
{
  if (idx < sib1_fragments.size()) {
    return &sib1_fragments[idx];
  }
  return nullptr;
}
//89da36:end
