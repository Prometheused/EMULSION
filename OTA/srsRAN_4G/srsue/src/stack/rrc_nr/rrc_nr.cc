...
void rrc_nr::decode_pdu_bcch_dlsch(srsran::unique_byte_buffer_t pdu)
{
  ...
  logger.info("=== 89da36: decode_pdu_bcch_dlsch called ===");
  ...
  std::string msg_type_str = dlsch_msg.msg.c1().type().to_string();
  logger.info("=== 89da36: Message type check, c1 type = %s ===", msg_type_str.c_str());

  if (dlsch_msg.msg.c1().type() == bcch_dl_sch_msg_type_c::c1_c_::types::sib_type1) {
    logger.info("=== 89da36: SIB1 type detected! ===");
    //89da36:start - Process SIB1 (handle_sib1 will notify cell selection procedure)
    size_t fragment_count_before = meas_cells.serving_cell().get_nof_sib1_fragments();
    logger.info("=== 89da36 DEBUG: Processing SIB1 (%zu/%u) ===", fragment_count_before + 1, args.expected_sib1_fragments);
    logger.info("=== 89da36 DEBUG: About to call handle_sib1 ===");
    handle_sib1(dlsch_msg.msg.c1().sib_type1());
    logger.info("=== 89da36 DEBUG: Returned from handle_sib1 ===");
    // handle_sib1 will call cell_selector.trigger(true) when needed
    //89da36:end
  }
}
...
{
  //89da36:start - Support for collecting multiple SIB1 fragments
  // Expected number of SIB1 fragments (should match NUM_SIB1_FRAGMENTS in gNB)
  // Value is read from config file: rat.nr.expected_sib1_fragments
  
  logger.info("handle_sib1 called: has_sib1=%d, current_fragments=%zu", 
              meas_cells.serving_cell().has_sib1(), 
              meas_cells.serving_cell().get_nof_sib1_fragments());
  
  // Check if this is the first SIB1 (for PHY/MAC configuration)
  bool is_first_sib1 = not meas_cells.serving_cell().has_sib1();
  
  int8_t q_rx_lev_min = sib1.cell_sel_info_present ? sib1.cell_sel_info.q_rx_lev_min : 0;
  logger.info("handle_sib1: is_first_sib1=%d, q_rx_lev_min=%d", is_first_sib1, q_rx_lev_min);
  
  // Try to add this SIB1 fragment to the collection
  bool is_new_fragment = meas_cells.serving_cell().add_sib1_fragment(sib1);
  
  logger.info("handle_sib1: add_sib1_fragment returned is_new_fragment=%d, fragment_count=%zu", 
              is_new_fragment, meas_cells.serving_cell().get_nof_sib1_fragments());
  
  if (is_new_fragment) {
    size_t fragment_count = meas_cells.serving_cell().get_nof_sib1_fragments();
    int8_t q_rx_lev_min = sib1.cell_sel_info_present ? sib1.cell_sel_info.q_rx_lev_min : 0;
    logger.info("SIB1 fragment received (%zu/%u), q_rx_lev_min=%d, CellID=%d",
                fragment_count, args.expected_sib1_fragments, q_rx_lev_min,
                meas_cells.serving_cell().get_cell_id() & 0xfff);
    {
      srsran::tti_point current_tti = stack->get_current_tti();
      std::printf("[UE]  SIB1 RECVD (fragment %zu/%zu) | SFN: %u | Subframe: %u | SFN-Time: %u ms\n",
                  fragment_count, (size_t)args.expected_sib1_fragments,
                  current_tti.sfn(), current_tti.sf_idx(),
                  current_tti.sfn() * 10 + current_tti.sf_idx());
      std::fflush(stdout);
    }
    
    // If this is the first SIB1, set it as the primary one and configure PHY/MAC
    if (is_first_sib1) {
      meas_cells.serving_cell().set_sib1(sib1);
      logger.info("SIB1 received, CellID=%d", meas_cells.serving_cell().get_cell_id() & 0xfff);
    } else {
      // For subsequent fragments, notify cell selector immediately
      // Always trigger, even if not busy, to ensure the procedure checks fragment count
      logger.info("Triggering cell selector for fragment %zu/%u", fragment_count, args.expected_sib1_fragments);
      cell_selector.trigger(true);
      logger.debug("Notified cell selector about new SIB1 fragment");
      return;
    }
  } else {
    int8_t q_rx_lev_min = sib1.cell_sel_info_present ? sib1.cell_sel_info.q_rx_lev_min : 0;
    logger.debug("Duplicate SIB1 fragment received (already in collection), q_rx_lev_min=%d", q_rx_lev_min);
    // Don't process duplicate fragments, but still notify cell selector
    logger.debug("Triggering cell selector for duplicate fragment");
    cell_selector.trigger(true);
    return;
  }
  //89da36:end
