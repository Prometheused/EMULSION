...
proc_outcome_t rrc_nr::cell_selection_proc::react(const bool sib1_found)
{
  ...
  unsigned expected_fragments = rrc_handle.args.expected_sib1_fragments;
  
  size_t fragment_count = rrc_handle.meas_cells.serving_cell().get_nof_sib1_fragments();
  
  Info("Cell selection procedure triggered: sib1_found=%d, current_fragments=%zu, expected=%u", 
       sib1_found, fragment_count, expected_fragments);
  
  if (fragment_count < expected_fragments) {
    Info("SIB1 fragment received (%zu/%u), continuing to collect more fragments...", 
         fragment_count, expected_fragments);
    // Continue BCCH search to collect more fragments
    rrc_handle.mac->bcch_search(true);
    return proc_outcome_t::yield;
  }
  
  Info("All %u SIB1 fragments acquired successfully!", expected_fragments);
  //89da36:end
