...
enum class sib_type { sib1 = 1, sib2 = 2, sib3 = 3, sib19 = 19, sib_invalid };
...
/// \brief SIB3 configuration — intra-frequency neighbour and excluded cell lists (TS 38.331).
/// Both lists may be empty, which produces a valid minimal SIB3.
struct sib3_info {
  sib3_info() {}
  /// Physical Cell Identities of intra-frequency neighbour cells. Values {0,...,1007}.
  std::vector<uint16_t> intra_freq_neigh_cell_pcis;
  /// Physical Cell Identities of intra-frequency excluded cells (cells UE shall not camp on). Values {0,...,1007}.
  std::vector<uint16_t> intra_freq_excluded_cell_pcis;
};
...
using sib_info = std::variant<sib2_info, sib3_info, sib19_info>;
