...
// ─── EMSS MAYO-2 gNB file-scope state ────────────────────────────────────────
#pragma pack(push, 1)
struct EmssBase {
  uint32_t idx;
  uint8_t  msg[79];
...
static const uint8_t  EMSS_STREAM_ID[]  = "EMSS-STREAM-V1";
static constexpr size_t EMSS_SID_LEN    = 14;
static constexpr size_t EMSS_SIGPLD_LEN = EMSS_SID_LEN + 4 + 32 + 32; // 82
static constexpr size_t MAYO2_SIG_LEN   = 180; // MAYO-2 fixed signature length
static constexpr size_t RSA1024_SIG_LEN = 128; // RSA-1024 fixed signature length

struct EmssGnbState {
  OQS_SIG*             oqs  = nullptr;
  std::vector<uint8_t> pk, sk;
  uint8_t              h_chain[2][32];
  uint32_t             seq  = 0;
  bool                 initialized = false;
};
static EmssGnbState g_emss_gnb;

static void emss_sha256_gnb(const void* data, size_t len, uint8_t out[32]) {
  SHA256_CTX ctx;
  SHA256_Init(&ctx);
...
static EmssRsaGnbState g_emss_rsa_gnb;
// ─── end EMSS gNB state ───────────────────────────────────────────────────────

// ─── EMULSION (TESLA/MAYO-2) gNB state ───────────────────────────────────────
static constexpr int    EMULSION_KEY_LEN  = 32;
static constexpr int    EMULSION_TAG_LEN  = 16;
static constexpr int    EMULSION_INFO_LEN = 48;
static constexpr int    EMULSION_CERT_LEN = 180;  // MAYO-2 fixed sig length
static constexpr int    EMULSION_CHAIN_MAX = 2;   // C=2

static const uint8_t EMULSION_CHAIN_PRF_KEY[32] = {
  0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,
  0x18,0x19,0x1a,0x1b,0x1c,0x1d,0x1e,0x1f,
  0x20,0x21,0x22,0x23,0x24,0x25,0x26,0x27,
  0x28,0x29,0x2a,0x2b,0x2c,0x2d,0x2e,0x2f
};
static const uint8_t EMULSION_DERIVE_PRF_KEY[32] = {
...
struct EmulsionGnbState {
  bool     initialized = false;
  uint8_t  chain[EMULSION_CHAIN_MAX + 1][EMULSION_KEY_LEN]; // K_0 .. K_ℓ
  uint8_t  info[EMULSION_INFO_LEN];
  uint8_t  cert[EMULSION_CERT_LEN];
  uint32_t id_bs;
  uint16_t d;
  uint16_t ell;
  uint32_t seq;
};
static EmulsionGnbState g_emulsion_gnb;

static void emulsion_hmac_sha256_gnb(const uint8_t* key, size_t klen,
                                      const uint8_t* msg, size_t mlen, uint8_t out[32]) {
  unsigned int olen = 0;
  HMAC(EVP_sha256(), key, (int)klen, msg, mlen, out, &olen);
}

static void emulsion_chain_F_gnb(const uint8_t in[32], uint8_t out[32]) {
  emulsion_hmac_sha256_gnb(EMULSION_CHAIN_PRF_KEY, 32, in, EMULSION_KEY_LEN, out);
}

static void emulsion_Fprime_gnb(const uint8_t Ki[32], uint8_t out[32]) {
  emulsion_hmac_sha256_gnb(EMULSION_DERIVE_PRF_KEY, 32, Ki, EMULSION_KEY_LEN, out);
}

static void emulsion_tag_gnb(const uint8_t mac_key[32], const uint8_t* msg,
                              uint32_t i, uint32_t id_bs, uint8_t tag[EMULSION_TAG_LEN]) {
  uint8_t buf[79 + 4 + 4];
  memcpy(buf,      msg,    79);
  memcpy(buf + 79, &i,     4);
  memcpy(buf + 83, &id_bs, 4);
  uint8_t full[32];
  emulsion_hmac_sha256_gnb(mac_key, 32, buf, sizeof(buf), full);
  memcpy(tag, full, EMULSION_TAG_LEN);
}
// ─── end EMULSION gNB state ───────────────────────────────────────────────────
...
SRSASN_CODE sib1_s::pack(bit_ref& bref) const
{
  ...
  // ── EMSS MAYO-2 multicast signing ──────────────────────────────────────────
  if (method_emss_mayo_present) {
    if (!g_emss_gnb.initialized) {
      g_emss_gnb.oqs = OQS_SIG_new("MAYO-2");
      if (g_emss_gnb.oqs) {
        g_emss_gnb.pk.resize(g_emss_gnb.oqs->length_public_key);
        g_emss_gnb.sk.resize(g_emss_gnb.oqs->length_secret_key);
        if (OQS_SIG_keypair(g_emss_gnb.oqs, g_emss_gnb.pk.data(), g_emss_gnb.sk.data()) == OQS_SUCCESS) {
          FILE* pkf = fopen("/tmp/mayo_emss_pk.bin", "wb");
          if (pkf) { fwrite(g_emss_gnb.pk.data(), 1, g_emss_gnb.pk.size(), pkf); fclose(pkf); }
          memset(g_emss_gnb.h_chain, 0, sizeof(g_emss_gnb.h_chain));
          g_emss_gnb.initialized = true;
          fmt::print("EMSS gNB: MAYO-2 initialized, pk={} bytes, written to /tmp/mayo_emss_pk.bin\n",
                     g_emss_gnb.pk.size());
        }
      }
    }
    if (g_emss_gnb.initialized) {
      g_emss_gnb.seq++;
      uint32_t seq    = g_emss_gnb.seq;
      bool     is_sig = (seq % 4 == 0);

      // Reset hash chain at epoch start (every sig_interval+1 = 4 packets)
      if (seq % 4 == 1) {
        memset(g_emss_gnb.h_chain, 0, sizeof(g_emss_gnb.h_chain));
      }
      ...
      EmssBase base{};
      base.idx = seq;
      // Capture first 79 bytes of packed SIB1 as the authenticated message
      {
        std::vector<unsigned char> msg_bytes;
        auto buf_opt = srsran::byte_buffer::from_view(bref.data());
        if (buf_opt) {
          srsran::byte_buffer buf = std::move(buf_opt.value());
          msg_bytes = srsran::byte_buffer::byte_buffer_to_char(buf);
        }
        size_t copy_n = std::min(msg_bytes.size(), (size_t)79);
        memcpy(base.msg, msg_bytes.data(), copy_n);
      }
      memcpy(base.h_prev1, g_emss_gnb.h_chain[0], 32);
      memcpy(base.h_prev2, g_emss_gnb.h_chain[1], 32);

      // Pack type byte
      HANDLE_CODE(pack_integer(bref, (int)(is_sig ? 'S' : 'P'), 0, 255));
      // Pack idx (4 bytes little-endian)
      for (int i = 0; i < 4; i++)
        HANDLE_CODE(pack_integer(bref, (int)((seq >> (8 * i)) & 0xFF), 0, 255));
      // Pack h_prev1 (32 bytes)
      for (int i = 0; i < 32; i++)
        HANDLE_CODE(pack_integer(bref, (int)base.h_prev1[i], 0, 255));
      // Pack h_prev2 (32 bytes)
      for (int i = 0; i < 32; i++)
        HANDLE_CODE(pack_integer(bref, (int)base.h_prev2[i], 0, 255));

      // Always pack MAYO2_SIG_LEN bytes: real sig for SIG packets, zeros for DATA.
      // This makes all fragments identical size so TBS is uniform across all 12 fragments.
      std::vector<uint8_t> sig_field(MAYO2_SIG_LEN, 0);
      if (is_sig) {
        uint8_t payload[EMSS_SIGPLD_LEN] = {};
        size_t  off = 0;
        memcpy(payload + off, EMSS_STREAM_ID, EMSS_SID_LEN); off += EMSS_SID_LEN;
        memcpy(payload + off, &seq, 4);                       off += 4;
        memcpy(payload + off, base.h_prev1, 32);              off += 32;
        memcpy(payload + off, base.h_prev2, 32);
        size_t sig_len = 0;
        OQS_SIG_sign(g_emss_gnb.oqs, sig_field.data(), &sig_len, payload, EMSS_SIGPLD_LEN,
                     g_emss_gnb.sk.data());
        fmt::print("EMSS gNB: SIG seq={} siglen={}\n", seq, sig_len);
      } else {
        fmt::print("EMSS gNB: DATA seq={}\n", seq);
      }
      for (size_t i = 0; i < MAYO2_SIG_LEN; i++)
        HANDLE_CODE(pack_integer(bref, (int)sig_field[i], 0, 255));

      // Advance hash chain: shift and store H(base)
      uint8_t h_cur[32];
      emss_sha256_gnb(&base, sizeof(EmssBase), h_cur);
      memcpy(g_emss_gnb.h_chain[1], g_emss_gnb.h_chain[0], 32);
      memcpy(g_emss_gnb.h_chain[0], h_cur, 32);
    }
  }
  // ── EMSS RSA-1024 multicast signing ────────────────────────────────────────
  if (method_emss_rsa_present) {
    if (!g_emss_rsa_gnb.initialized) {
      BIGNUM* e = BN_new();
      BN_set_word(e, RSA_F4);
      g_emss_rsa_gnb.rsa = RSA_new();
      if (RSA_generate_key_ex(g_emss_rsa_gnb.rsa, 1024, e, nullptr) == 1) {
        // DER-encode public key and write to file for UE
        int der_len = i2d_RSAPublicKey(g_emss_rsa_gnb.rsa, nullptr);
        g_emss_rsa_gnb.pk_der.resize(der_len);
        uint8_t* tmp = g_emss_rsa_gnb.pk_der.data();
        i2d_RSAPublicKey(g_emss_rsa_gnb.rsa, &tmp);
        FILE* pkf = fopen("/tmp/rsa_emss_pk.der", "wb");
        if (pkf) { fwrite(g_emss_rsa_gnb.pk_der.data(), 1, der_len, pkf); fclose(pkf); }
        memset(g_emss_rsa_gnb.h_chain, 0, sizeof(g_emss_rsa_gnb.h_chain));
        g_emss_rsa_gnb.initialized = true;
        fmt::print("EMSS-RSA gNB: RSA-1024 initialized, pk={} bytes, written to /tmp/rsa_emss_pk.der\n", der_len);
      }
      BN_free(e);
    }
    if (g_emss_rsa_gnb.initialized) {
      g_emss_rsa_gnb.seq++;
      uint32_t seq    = g_emss_rsa_gnb.seq;
      bool     is_sig = (seq % 4 == 0);

      // Reset hash chain at epoch start
      if (seq % 4 == 1) {
        memset(g_emss_rsa_gnb.h_chain, 0, sizeof(g_emss_rsa_gnb.h_chain));
      }
      ...
      memcpy(base.h_prev1, g_emss_rsa_gnb.h_chain[0], 32);
      memcpy(base.h_prev2, g_emss_rsa_gnb.h_chain[1], 32);
      ...
      // Always pack RSA1024_SIG_LEN bytes: real sig for SIG packets, zeros for DATA.
      std::vector<uint8_t> sig_field(RSA1024_SIG_LEN, 0);
      if (is_sig) {
        uint8_t payload[EMSS_SIGPLD_LEN] = {};
        size_t  off = 0;
        memcpy(payload + off, EMSS_STREAM_ID, EMSS_SID_LEN); off += EMSS_SID_LEN;
        ...
        memcpy(payload + off, base.h_prev2, 32);
        uint8_t digest[32];
        SHA256(payload, EMSS_SIGPLD_LEN, digest);
        unsigned int sig_len = 0;
        RSA_sign(NID_sha256, digest, 32, sig_field.data(), &sig_len, g_emss_rsa_gnb.rsa);
        fmt::print("EMSS-RSA gNB: SIG seq={} siglen={}\n", seq, sig_len);
      } else {
        fmt::print("EMSS-RSA gNB: DATA seq={}\n", seq);
      }
      for (size_t i = 0; i < RSA1024_SIG_LEN; i++)
        HANDLE_CODE(pack_integer(bref, (int)sig_field[i], 0, 255));

      // Advance hash chain
      uint8_t h_cur[32];
      emss_sha256_gnb(&base, sizeof(EmssBase), h_cur);
      memcpy(g_emss_rsa_gnb.h_chain[1], g_emss_rsa_gnb.h_chain[0], 32);
      memcpy(g_emss_rsa_gnb.h_chain[0], h_cur, 32);
    }
  }
  // ── end EMSS ───────────────────────────────────────────────────────────────

  // ── EMULSION (TESLA/MAYO-2) signing ─────────────────────────────────────────
  if (method_emulsion_present) {
    // ---- EMULSION.Setup (AMF role): run once ----
    if (!g_emulsion_gnb.initialized) {
      bool loaded = false;
      FILE* cf    = fopen("/tmp/emulsion_chain.bin",  "rb");
      FILE* inf   = fopen("/tmp/emulsion_info.bin",   "rb");
      FILE* certf = fopen("/tmp/emulsion_cert_K0.bin","rb");
      if (cf && inf && certf) {
        fread(g_emulsion_gnb.chain, EMULSION_KEY_LEN, EMULSION_CHAIN_MAX + 1, cf);
        fread(g_emulsion_gnb.info,  1, EMULSION_INFO_LEN, inf);
        fread(g_emulsion_gnb.cert,  1, EMULSION_CERT_LEN, certf);
        loaded = true;
        fmt::print("EMULSION gNB: loaded chain/info/cert from /tmp/emulsion_*.bin\n");
      }
      if (cf)    fclose(cf);
      if (inf)   fclose(inf);
      if (certf) fclose(certf);

      if (!loaded) {
        fmt::print("EMULSION gNB: running EMULSION.Setup (AMF role)...\n");
        OQS_SIG* sig = OQS_SIG_new("MAYO-2");
        if (sig) {
          size_t   pk_len  = sig->length_public_key;
          uint8_t* pk_amf  = (uint8_t*)malloc(pk_len);
          uint8_t* sk_amf  = (uint8_t*)malloc(sig->length_secret_key);
          OQS_SIG_keypair(sig, pk_amf, sk_amf);

          // K_ℓ ← random; K_i = F(K_{i+1}) for i = ℓ−1 down to 0
          RAND_bytes(g_emulsion_gnb.chain[EMULSION_CHAIN_MAX], EMULSION_KEY_LEN);
          for (int k = EMULSION_CHAIN_MAX - 1; k >= 0; k--)
            emulsion_chain_F_gnb(g_emulsion_gnb.chain[k + 1], g_emulsion_gnb.chain[k]);

          // info = K_0 || ID_BS(4) || T_0(4) || T_int(4) || d(2) || ℓ(2)
          uint32_t id_bs_v = 1, t0_v = 0, t_int_v = 160;
          uint16_t d_v = 1, ell_v = (uint16_t)EMULSION_CHAIN_MAX;
          memset(g_emulsion_gnb.info, 0, EMULSION_INFO_LEN);
          memcpy(g_emulsion_gnb.info,      g_emulsion_gnb.chain[0], EMULSION_KEY_LEN);
          memcpy(g_emulsion_gnb.info + 32, &id_bs_v, 4);
          memcpy(g_emulsion_gnb.info + 36, &t0_v,    4);
          memcpy(g_emulsion_gnb.info + 40, &t_int_v, 4);
          memcpy(g_emulsion_gnb.info + 44, &d_v,     2);
          memcpy(g_emulsion_gnb.info + 46, &ell_v,   2);

          // cert_K0 = MAYO.Sign(sk_AMF, info)
          size_t cert_out_len = 0;
          OQS_SIG_sign(sig, g_emulsion_gnb.cert, &cert_out_len,
                       g_emulsion_gnb.info, EMULSION_INFO_LEN, sk_amf);

          // Persist for BS (reload avoidance) and share PK with UE
          FILE* of;
          of = fopen("/tmp/emulsion_chain.bin",   "wb");
          if (of) { fwrite(g_emulsion_gnb.chain, EMULSION_KEY_LEN, EMULSION_CHAIN_MAX + 1, of); fclose(of); }
          of = fopen("/tmp/emulsion_info.bin",    "wb");
          if (of) { fwrite(g_emulsion_gnb.info,  1, EMULSION_INFO_LEN, of); fclose(of); }
          of = fopen("/tmp/emulsion_cert_K0.bin", "wb");
          if (of) { fwrite(g_emulsion_gnb.cert,  1, cert_out_len, of); fclose(of); }
          of = fopen("/tmp/emulsion_pk_amf.bin",  "wb");  // UE eSIM provisioning
          if (of) { fwrite(pk_amf, 1, pk_len, of); fclose(of); }

          fmt::print("EMULSION gNB: Setup done. cert_len={}, pk_len={}\n", cert_out_len, pk_len);
          free(pk_amf); free(sk_amf);
          OQS_SIG_free(sig);
          loaded = true;
        } else {
          fmt::print("EMULSION gNB: MAYO-2 unavailable!\n");
        }
      }
      if (loaded) {
        memcpy(&g_emulsion_gnb.id_bs, g_emulsion_gnb.info + 32, 4);
        memcpy(&g_emulsion_gnb.d,     g_emulsion_gnb.info + 44, 2);
        memcpy(&g_emulsion_gnb.ell,   g_emulsion_gnb.info + 46, 2);
        g_emulsion_gnb.seq = 0;
        g_emulsion_gnb.initialized = true;
      }
    }

    if (g_emulsion_gnb.initialized) {
      g_emulsion_gnb.seq++;
      // Interval i cycles 1..ℓ
      uint32_t i = ((g_emulsion_gnb.seq - 1) % g_emulsion_gnb.ell) + 1;

      // Capture first 79 bytes of packed SIB1 as authenticated message m_i
      uint8_t sib1_msg[79] = {};
      {
        std::vector<unsigned char> msg_bytes;
        auto buf_opt = srsran::byte_buffer::from_view(bref.data());
        ...
        size_t n = std::min(msg_bytes.size(), (size_t)79);
        memcpy(sib1_msg, msg_bytes.data(), n);
      }

      // K'_i = F'(K_i);  τ_i = HMAC(K'_i, m_i || i || ID_BS)
      uint8_t mac_key[32], tag[EMULSION_TAG_LEN];
      emulsion_Fprime_gnb(g_emulsion_gnb.chain[i], mac_key);
      emulsion_tag_gnb(mac_key, sib1_msg, i, g_emulsion_gnb.id_bs, tag);

      if (i == 1) {
        // ---- Epoch-opening packet P_1 ----
        // Wire: 'E'(1) | i(4 LE) | τ(16) | info(48) | cert(180)  = 249 bytes overhead
        HANDLE_CODE(pack_integer(bref, (int)'E', 0, 255));
        for (int b = 0; b < 4; b++)
          HANDLE_CODE(pack_integer(bref, (int)((i >> (8 * b)) & 0xFF), 0, 255));
        for (int b = 0; b < EMULSION_TAG_LEN; b++)
          HANDLE_CODE(pack_integer(bref, (int)tag[b], 0, 255));
        for (int b = 0; b < EMULSION_INFO_LEN; b++)
          HANDLE_CODE(pack_integer(bref, (int)g_emulsion_gnb.info[b], 0, 255));
        for (int b = 0; b < EMULSION_CERT_LEN; b++)
          HANDLE_CODE(pack_integer(bref, (int)g_emulsion_gnb.cert[b], 0, 255));
        fmt::print("EMULSION gNB: P1 epoch-opening packed (i=1, overhead=249 B)\n");
      } else {
        // ---- Subsequent packet P_i ----
        // K_disc = K_{i-d};  Wire: 'S'(1) | i(4 LE) | τ(16) | K_disc(32) = 53 bytes overhead
        uint32_t disc = i - (uint32_t)g_emulsion_gnb.d;
        HANDLE_CODE(pack_integer(bref, (int)'S', 0, 255));
        for (int b = 0; b < 4; b++)
          HANDLE_CODE(pack_integer(bref, (int)((i >> (8 * b)) & 0xFF), 0, 255));
        for (int b = 0; b < EMULSION_TAG_LEN; b++)
          HANDLE_CODE(pack_integer(bref, (int)tag[b], 0, 255));
        for (int b = 0; b < EMULSION_KEY_LEN; b++)
          HANDLE_CODE(pack_integer(bref, (int)g_emulsion_gnb.chain[disc][b], 0, 255));
        fmt::print("EMULSION gNB: P{} subsequent packed (i={}, K_disc=K_{}, overhead=53 B)\n",
                   i, i, disc);
      }
    }
  }
