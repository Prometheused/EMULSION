...
static asn1::rrc_nr::sib1_s make_asn1_rrc_cell_sib1(const du_cell_config& du_cfg)
{
  ...
  if (du_cfg.si_config.has_value()) {
    for (const auto& sib : du_cfg.si_config->sibs) {
      if (std::holds_alternative<sib2_info>(sib) || std::holds_alternative<sib3_info>(sib)) {
...
static asn1::rrc_nr::sib3_s make_asn1_rrc_cell_sib3(const sib3_info& sib3_params)
{
  using namespace asn1::rrc_nr;
  sib3_s sib3;
  // Populate intra-frequency neighbour cell list.
  for (uint16_t pci : sib3_params.intra_freq_neigh_cell_pcis) {
    intra_freq_neigh_cell_info_s entry;
    entry.pci            = pci;
    entry.q_offset_cell  = q_offset_range_opts::db0;
    sib3.intra_freq_neigh_cell_list.push_back(entry);
  }
  // Populate intra-frequency excluded cell list.
  for (uint16_t pci : sib3_params.intra_freq_excluded_cell_pcis) {
    pci_range_s entry;
    entry.start = pci;
    sib3.intra_freq_excluded_cell_list.push_back(entry);
  }
  return sib3;
}
...
std::vector<byte_buffer> srsran::srs_du::make_asn1_rrc_cell_bcch_dl_sch_msgs(const du_cell_config& du_cfg)
{
  ...
  // Pack SIB1.
  //89da36:start - Configurable number of SIB1 fragments
  // Number of SIB1 fragments to create. Value is read from config file: cell_cfg.num_sib1_fragments
  // Each fragment will have a slightly different q_rx_lev_min value to distinguish them in Wireshark.
  unsigned num_sib1_fragments = du_cfg.num_sib1_fragments;
  //89da36:end
  
  {
    for (unsigned frag_idx = 0; frag_idx < num_sib1_fragments; ++frag_idx) {
      byte_buffer                     buf;
      asn1::bit_ref                   bref{buf};
      asn1::rrc_nr::bcch_dl_sch_msg_s msg;
      
      // Create SIB1 - modify it if this is not the first fragment
      asn1::rrc_nr::sib1_s sib1 = make_asn1_rrc_cell_sib1(du_cfg);
      
      if (frag_idx > 0) {
        // Modify q_rx_lev_min in cell selection info to make each fragment distinguishable
        // This field is visible in Wireshark under "Cell Selection Info" and won't break functionality
        if (sib1.cell_sel_info_present) {
          // Change q_rx_lev_min by adding (4 * frag_idx) for each fragment.
          // Step=4 keeps all 12 fragments within the valid ASN.1 range [-70, -22].
          // With original_value=-70: frag 0=-70, frag 1=-66, ..., frag 11=-26 (all ≤ -22).
          int8_t original_value = sib1.cell_sel_info.q_rx_lev_min;
          sib1.cell_sel_info.q_rx_lev_min = original_value + (4 * frag_idx);
          fmt::print("Modified SIB1 fragment {}: q_rx_lev_min changed from {} to {} (fragment 0 has {})\n",
                     frag_idx, original_value, sib1.cell_sel_info.q_rx_lev_min, original_value);
        }
      } else {
        fmt::print("----------------------------Packing SIB1 fragment {}-----------------\n", frag_idx);
      }
      
      msg.msg.set_c1().set_sib_type1() = std::move(sib1);
      asn1::SRSASN_CODE ret            = msg.pack(bref);
      srsran_assert(ret == asn1::SRSASN_SUCCESS, "Failed to pack SIB1 fragment {}", frag_idx);
