...
{
  //fmt::print("Inside MAC SLOT INDICATION method\n");
                                                  ...
                                                  const dl_sched_result& dl_res)
{
  ...
  for (const sib_information& sib_info : dl_res.bc.sibs) {
    ...
    if (sib_info.si_indicator == sib_information::sib1) {
      ...
      sib1_count++;
      logger.debug("Assembling SIB1 PDU #{} for slot {}", sib1_count, sl_tx);
    } else {
  ...
  if (sib1_count > 0) {
    logger.info("Assembled {} SIB1 PDU(s) for slot {}", sib1_count, sl_tx);
  }
                                           ...
                                           const mac_dl_data_result& dl_res)
{
  ...
  for (unsigned i = 0, e = dl_res.si_pdus.size(); i != e; ++i) {
    ...
    // Write all SIB1 messages to PCAP (not just the first one)
    // This allows multiple SIB1 messages to be captured for analysis
    if (dl_alloc.si_indicator == sib_information::sib1) {
      ...
      logger.debug("Wrote SIB1 PDU to PCAP for slot {}", sl_tx);
    }
  }
