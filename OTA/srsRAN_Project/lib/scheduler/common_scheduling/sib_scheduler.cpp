...
void sib1_scheduler::schedule_sib1(cell_slot_resource_allocator& res_grid)
{
  ...
  for (unsigned ssb_idx = 0; ssb_idx < MAX_NUM_BEAMS; ssb_idx++) {
    ...
    // Check if this is a slot where SIB1 should be scheduled
    // Original logic: schedule SIB1 when slot matches the base slot in the period
    // We extend this to also schedule in the next consecutive slot for multiple SIB1 support
    unsigned base_slot_abs = sib1_type0_pdcch_css_slots[ssb_idx].to_uint();
    unsigned current_slot_abs = sl_point.to_uint();
    unsigned base_slot_in_period = base_slot_abs % sib1_rtx_period_slots;
    unsigned current_slot_in_period = current_slot_abs % sib1_rtx_period_slots;
    
    // Check if this slot matches the primary SIB1 slot or the next consecutive slot
    bool is_primary_slot = (current_slot_in_period == base_slot_in_period);
    unsigned next_slot_in_period = (base_slot_in_period + 1) % sib1_rtx_period_slots;
    bool is_consecutive_slot = (current_slot_in_period == next_slot_in_period);
    
    logger.info("SIB1 slot check for beam {}: slot={}, base_slot_in_period={}, current_slot_in_period={}, "
                "next_slot_in_period={}, is_primary={}, is_consecutive={}",
                ssb_idx, sl_point, base_slot_in_period, current_slot_in_period, 
                next_slot_in_period, is_primary_slot, is_consecutive_slot);
    
    if (not is_primary_slot and not is_consecutive_slot) {
      continue;  // Not a slot for SIB1
    }
    ...
    // Only apply pending updates on the primary slot (first slot in the period)
    if ((current_slot_abs % sib1_newtx_period_slots) == base_slot_abs && is_primary_slot) {
      // If it is a new Tx (first slot), apply a new SIB1 PDU version, if it is pending.
    ...
    // Schedule ONE SIB1 per slot. For multiple SIB1 support, we schedule in consecutive slots
    // (primary slot and next slot), and the MAC layer's fragmentation logic will cycle through
    // different SIB1 messages automatically.
    ...
    for (const auto& pdsch_td_res : pdsch_td_res_alloc_list) {
      ...
      if (pdsch_td_res.symbols.start() >= ss_cfg.get_first_symbol_index(ssb_idx) + coreset_duration) {
        ...
        if (allocate_sib1(res_grid, ssb_idx, time_resource)) {
          ...
          bool is_new_tx = ((current_slot_abs % sib1_newtx_period_slots) ==
                            (base_slot_abs % sib1_newtx_period_slots)) && is_primary_slot;
          logger.info("Scheduled SIB1 for beam idx {} in slot {} (primary={}, consecutive={}, new_tx={})",
                     ssb_idx, sl_point, is_primary_slot, is_consecutive_slot, is_new_tx);
          if (is_new_tx) {
            // std::printf("[gNB] SIB1 SENT (new TX) | SFN: %u | Slot-in-frame: %u | Subframe: %u | SFN-Time: %u ms\n",
            //             sl_point.sfn(), sl_point.slot_index(), sl_point.subframe_index(),
            //             sl_point.sfn() * 10 + sl_point.subframe_index());
            // std::fflush(stdout);
          }
          break;  // Only schedule one SIB1 per slot
        }
      }
    }
    
    if (not is_sib1_scheduled) {
      logger.error("No valid PDSCH time domain resource exists to schedule SIB1 for beam idx {}", ssb_idx);
    }
  }
}
...
bool sib1_scheduler::allocate_sib1(cell_slot_resource_allocator& res_grid, unsigned beam_idx, unsigned time_resource)
{
  ...
  // Check if PDCCH monitoring is active in this slot
  const search_space_configuration& ss_cfg_check =
      cell_cfg.dl_cfg_common.init_dl_bwp.pdcch_common
          .search_spaces[cell_cfg.dl_cfg_common.init_dl_bwp.pdcch_common.sib1_search_space_id];
  ...
  if (pdcch == nullptr) {
    logger.warning("Could not allocated SIB1's DCI in PDCCH for beam idx: {} in slot: {}. "
                   "This may be because PDCCH monitoring is not active in this slot.",
                   beam_idx, res_grid.slot);
    return false;
  }
